...
<div><p>{taskDummy.name}</p></div>
			...
			<div className="flex flex-row mt-10 items-end justify-end">
				<div className="inline-block  hidden relative w-64">
					...
					<div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
						<svg className="fill-black text-black h- w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
        ...
        .map((post) => (
          <>
            {post.data.is_video && !post.data.media.is_gif ? (<div className="flex flex-1  hover:scale-[10px] flex-col h-56 bg-gray-300 bg-opacity-40 rounded-xl w-fit items-center">
<svg type="button"
xmlns="http://www.w3.org/2000/svg" onClick={colorChange} className={`h-6 ${color} duration-200 hover:fill-green-500 self-end  active:fill-purple-400 w-6` }viewBox="0 0 20 20" fill={color}>
  <path d="M5 4a2 2 0 012-2h6a2 2 0 012 2v14l-5-2.5L5 18V4z" />
</svg>
              <video className="rounded-lg h-[calc(100vh-20rem)]   w-full "
                controls
                src={post.data.media.reddit_video.fallback_url}
              />  <p className="my-2 text-lg border-b-2 mx-1 ">{post.data.title}</p>
														<a href={post.data.url}><button className="h-fit w-fit bg bg-purple-400 rounded-md border-2 font-subheading font-light border-opacity-40 hover:border-2 hover:border-green-400 p-2 my-2  scale-100 border-black px-1"> Take Me To The Link</button></a></div>
            ) : (<div className="flex flex-1    h-[calc(100vh-15rem)] flex-row grow ">
<div className="flex flex-col   relative ">
	<svg id="button1" onClick={colorChange}
	xmlns="http://www.w3.org/2000/svg" className="h-6  hover:scale-[100.8%] duration-200 z-10 top-5 left15  place-self-end  hover:fill-green-400  active:fill-purple-400 w-6" viewBox="0 0 20 20" fill={color}>
  <path d="M5 4a2 2 0 012-2h6a2 2 0 012 2v14l-5-2.5L5 18V4z" />
</svg><iframe className="border-2    relative  rounded-2xl flex   " id="reddit-embed embed-responsive"  src={`https://www.redditmedia.com/${post.data.permalink}?ref_source=embed&amp;ref=share&amp;embed=true;&autoplay=false` } sandbox="allow-scripts allow-orientation-lock allow-same-origin"  height="520" width="900"   do-not-allow="autoplay" scrolling="no"></iframe></div>
</div>
            )}
          </>
        ))}
